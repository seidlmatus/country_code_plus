...
- `lib/` contains the public package API and implementation. `lib/country_codes_plus.dart` is the main entry point; `lib/src/` holds internal helpers like `country_details.dart` and `dial_code_formatter.dart`.
...
- Keep public API surface in `lib/` minimal and re-export from `lib/country_codes_plus.dart`.
